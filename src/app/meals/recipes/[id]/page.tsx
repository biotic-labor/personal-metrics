...
export default function RecipeDetailPage() {
  ...
  function startEditing() {
    ...
    setEditMealType(recipe.mealType || '');
    setEditTotalTime(recipe.totalTimeMinutes?.toString() || '');
  ...
  async function saveEdits() {
    ...
    const res = await fetch(`/api/meals/recipes/${params.id}`, {
      ...
      body: JSON.stringify({
        ...
        totalTimeMinutes: editTotalTime,
  ...
  const hasPeanutAllergen = allergens.some(a => a.toLowerCase().includes('peanut'));
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-3">
        ...
        {editing ? (
          <input
            ...
            className="flex-1 rounded-md border border-zinc-300 bg-white px-3 py-1.5 text-xl font-bold text-zinc-900 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-100"
          />
        ) : (
      ...
      {editing ? (
        <textarea
          ...
          rows={2}
          className="w-full rounded-md border border-zinc-300 bg-white px-3 py-2 text-sm text-zinc-700 dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-300"
        />
      ) : recipe.description ? (
      ...
      {editing ? (
        <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
          <div>
            <label className="mb-1 block text-xs font-medium text-zinc-500">Time (min)</label>
            <input
              type="number"
              value={editTotalTime}
              onChange={e => setEditTotalTime(e.target.value)}
              className="w-full rounded-md border border-zinc-300 bg-white px-3 py-1.5 text-sm dark:border-zinc-600 dark:bg-zinc-800 dark:text-zinc-300"
            />
          </div>
          <div>
            <label className="mb-1 block text-xs font-medium text-zinc-500">Servings</label>
            <input
              type="number"
              value={editServings}
          ...
          <div>
            <label className="mb-1 block text-xs font-medium text-zinc-500">Cuisine</label>
            <input
              value={editCuisine}
        ...
        <div className="lg:col-span-1">
          ...
          {editing ? (
            <textarea
              ...
              rows={Math.max(8, editIngredients.split('\n').length + 2)}
        ...
        <div className="lg:col-span-2">
          ...
          {editing ? (
            <textarea
              ...
              rows={Math.max(8, editInstructions.split('\n').length + 2)}
